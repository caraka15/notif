...
// --- PENAMBAHAN VARIABEL GLOBAL UNTUK DATA EPOCH ---
let globalEpochProgress = null; // Akan menyimpan data progress epoch dari API
...
// --- BARU: FUNGSI UNTUK MENGAMBIL DATA EPOCH SECARA BERKALA ---
async function updateGlobalEpochData() {
    try {
        const response = await axios.get('https://validator.crxanode.xyz/api/dashboard', { timeout: 10000 });
        if (response.data && response.data.webServerEpochProgress) {
            globalEpochProgress = response.data.webServerEpochProgress;
        } else {
            await logMessage('Gagal memperbarui data epoch: struktur data API tidak sesuai.', 'WARN');
            globalEpochProgress = null;
        }
    } catch (error) {
        await logMessage(`Gagal mengambil data progress epoch dari API: ${error.message}`, 'ERROR');
        globalEpochProgress = null;
    }
}
...
async function loadAllowlist() {
    try {
        ...
        if (error.code === 'ENOENT') {
            ...
            const exampleAllowlist = { "6281234567890": { "ip": "IP_SERVER_ANDA", "port": "PORT_API_HELPER_ANDA", "name": "NamaServerAnda1", "address": "AlamatHumanodeWalletAnda_Format_SS58", "status": "active", "managedAdmin": 0 } };
    ...
    try {
        ...
        if (allowlist[phoneKey]) {
            ...
            Object.keys(allowlist[phoneKey]).forEach(key => {
                if (allowlist[phoneKey][key] === null) { delete allowlist[phoneKey][key]; }
            });
...
class HumanodeBalanceMonitor {
    constructor() { this.wsEndpoint = 'wss://explorer-rpc-ws.mainnet.stages.humanode.io'; this.api = null; this.lastBalances = new Map(); this.activeSubscriptions = new Map(); }
    async connect() { if (this.api && this.api.isConnected) { await logMessage('Sudah terhubung ke Jaringan Humanode.', 'BALANCE'); return true; } try { await logMessage('Menghubungkan ke Jaringan Humanode...', 'BALANCE'); const provider = new WsProvider(this.wsEndpoint); provider.on('connected', () => logMessage('Provider WS Humanode terhubung.', 'SYSTEM')); provider.on('disconnected', async () => { await logMessage('Provider WS Humanode terputus. Objek API akan dibersihkan.', 'WARN'); if (this.api) this.api = null; }); provider.on('error', (err) => logMessage(`Error Provider WS Humanode: ${err.message}`, 'ERROR')); this.api = await ApiPromise.create({ provider, ss58Format: 5234 }); await this.api.isReady; await logMessage('Berhasil terhubung ke Jaringan Humanode.', 'SUCCESS'); return true; } catch (error) { await logMessage(`Error koneksi API Humanode: ${error.message}`, 'ERROR'); this.api = null; return false; } }
    async getBalance(address) { try { if (!this.api || !this.api.isConnected) { if (!await this.connect()) throw new Error("Gagal terhubung ke API Humanode untuk getBalance."); } const { data: balance } = await this.api.query.system.account(address); const free = this.api.createType('Balance', balance.free); return { free: free.toString(), timestamp: new Date().toISOString() }; } catch (error) { await logMessage(`Error memeriksa saldo untuk ${address}: ${error.message}`, 'ERROR'); throw error; } }
    formatBalance(rawBalance) { const actualBalance = Number(BigInt(rawBalance)) / Number(BigInt(1_000_000_000_000_000_000)); return actualBalance.toFixed(2).replace('.', ','); }
    async stopMonitoringAddress(address) { if (this.activeSubscriptions.has(address)) { const unsubscribe = this.activeSubscriptions.get(address); if (typeof unsubscribe === 'function') { try { unsubscribe(); await logMessage(`Berhasil berhenti berlangganan pembaruan saldo untuk: ${address}`, 'BALANCE'); } catch (e) { await logMessage(`Error saat berhenti berlangganan untuk ${address}: ${e.message}`, 'ERROR'); } } this.activeSubscriptions.delete(address); this.lastBalances.delete(address); } }
    async stopAllMonitoring() { await logMessage('Menghentikan semua langganan pemantauan saldo aktif...', 'BALANCE'); const addressesToStop = Array.from(this.activeSubscriptions.keys()); for (const address of addressesToStop) await this.stopMonitoringAddress(address); if (addressesToStop.length > 0) await logMessage('Semua langganan saldo dihentikan.', 'SUCCESS'); else await logMessage('Tidak ada langganan saldo aktif untuk dihentikan.', 'INFO'); }
    async startMonitoring(phone, serverConfig) { try { if (!serverConfig.address || serverConfig.address === 'Tidak diset' || serverConfig.address.trim() === '') { await logMessage(`Lewati pemantauan saldo ${serverConfig.name}: Alamat tidak valid.`, 'BALANCE'); return; } if (!this.api || !this.api.isConnected) { if (!await this.connect()) { await logMessage(`Tidak dapat memulai pemantauan ${serverConfig.name}: Koneksi API Humanode gagal.`, 'ERROR'); return; } } if (this.activeSubscriptions.has(serverConfig.address)) { await logMessage(`Menghentikan monitor saldo lama untuk ${serverConfig.name} (${serverConfig.address}) sebelum memulai ulang.`, 'BALANCE'); await this.stopMonitoringAddress(serverConfig.address); } await logMessage(`Memulai pemantauan saldo ${serverConfig.name}: ${serverConfig.address}`, 'BALANCE'); const initialBalanceData = await this.getBalance(serverConfig.address); this.lastBalances.set(serverConfig.address, initialBalanceData.free); await logMessage(`Saldo awal ${serverConfig.name} (${serverConfig.address}): ${this.formatBalance(initialBalanceData.free)} HMND`, 'BALANCE'); const unsubscribe = await this.api.query.system.account(serverConfig.address, async (accountInfo) => { const currentBalance = accountInfo.data.free.toString(); const lastBalance = this.lastBalances.get(serverConfig.address); if (typeof lastBalance !== 'undefined' && BigInt(currentBalance) > BigInt(lastBalance)) { const difference = BigInt(currentBalance) - BigInt(lastBalance); if (difference > BigInt(0)) { const formattedAmount = this.formatBalance(difference.toString()); const message = `💰 *Pembaruan Saldo*\n\n*Server:* ${serverConfig.name}\n*Alamat:* ${serverConfig.address}\n*Saldo Masuk:* ${formattedAmount} HMND`; let targetRecipient = phone; const isAdminManaged = serverConfig.managedAdmin === 1; if (isAdminManaged) { if (ADMIN_WHATSAPP_NUMBER && ADMIN_WHATSAPP_NUMBER !== 'GANTI_DENGAN_NOMOR_ADMIN_628XXX' && /^\d+$/.test(ADMIN_WHATSAPP_NUMBER.replace(/@c\.us$/, ''))) { targetRecipient = ADMIN_WHATSAPP_NUMBER; await logMessage(`Server ${serverConfig.name} (${serverConfig.address}) dikelola admin. Notifikasi saldo akan dikirim ke Admin (${ADMIN_WHATSAPP_NUMBER}).`, 'BALANCE'); } else { await logMessage(`Server ${serverConfig.name} (${serverConfig.address}) dikelola admin, TAPI ADMIN_WHATSAPP_NUMBER ('${ADMIN_WHATSAPP_NUMBER}') tidak valid atau tidak dikonfigurasi. Notifikasi saldo akan dikirim ke user (${phone}).`, 'WARN'); } } await sendWhatsAppMessage(targetRecipient, message); await logMessage(`${serverConfig.name} (${serverConfig.address}) +${formattedAmount} HMND. Saldo baru: ${this.formatBalance(currentBalance)} HMND. Notifikasi terkirim ke ${targetRecipient.replace('@c.us', '')}`, 'BALANCE'); } } this.lastBalances.set(serverConfig.address, currentBalance); }); this.activeSubscriptions.set(serverConfig.address, unsubscribe); await logMessage(`Berhasil memulai pemantauan saldo ${serverConfig.name} (${serverConfig.address})`, 'SUCCESS'); } catch (error) { await logMessage(`Error memulai pemantauan saldo ${serverConfig.name} (${serverConfig.address}): ${error.message}`, 'ERROR'); } }
    async disconnect() { await logMessage('Memutus koneksi HumanodeBalanceMonitor...', 'BALANCE'); await this.stopAllMonitoring(); if (this.api && this.api.isConnected) { try { await this.api.disconnect(); await logMessage('API Humanode berhasil diputus.', 'SUCCESS'); } catch (e) { await logMessage(`Error memutus API Humanode: ${e.message}`, 'ERROR'); } } else { await logMessage('API Humanode sudah terputus atau belum diinisialisasi.', 'INFO'); } this.api = null; }
}
...
async function handleServerProblem(phone, serverConfig, problemType, details = {}) {
    ...
    // --- LOGIKA DEADLINE DINAMIS HANYA UNTUK "Inactive" ---
    if (problemType === "BioauthInactive") {
        if (globalEpochProgress && globalEpochProgress.estimatedEpochCompletionTime) {
            const epochEndTime = new Date(globalEpochProgress.estimatedEpochCompletionTime).getTime();
            ...
            deadlineMessage = `\n\n_Lakukan re-autentikasi maksimal ${deadlineText}_`;
        } else {
            deadlineMessage = "\n\n_Lakukan re-autentikasi dalam *2 jam kedepan*_";
        }
    }
...
// Fungsi Pengecek Bioauth (VERSI DIPERBAIKI)
async function checkBioauth(phone, serverConfig) {
    ...
    try {
        ...
        if (!data || !data.status || typeof data.status.result === 'undefined') {
            ...
            await handleServerProblem(phone, serverConfig, "InvalidResponse", { responseData: JSON.stringify(data), note: "Struktur API helper (/cek) tidak sesuai harapan." });
            return;
        }
        ...
        if (apiStatusResult === "ServerDown") {
            await logMessage(`${serverConfig.name} API Helper melaporkan status ServerDown. Memanggil handleServerProblem.`, 'BIOAUTH');
            await handleServerProblem(phone, serverConfig, "ServerDown");
            return;
        }

        if (apiStatusResult === "InvalidResponse") {
            await logMessage(`${serverConfig.name} API Helper melaporkan status InvalidResponse. Memanggil handleServerProblem.`, 'BIOAUTH');
            await handleServerProblem(phone, serverConfig, "InvalidResponse", { responseData: JSON.stringify(data), note: "API helper melaporkan InvalidResponse dari node Humanode." });
            return;
        }
        ...
        if (typeof apiStatusResult === 'object' && apiStatusResult.Active) {
            ...
            if (data.status.remaining_time) {
                if (typeof data.status.remaining_time.total_milliseconds === 'number' && data.status.remaining_time.total_milliseconds > 0) {
                    ...
                    remainingMillisecondsFromAPI = (data.status.remaining_time.hours * 3600000) + (data.status.remaining_time.minutes * 60000) + ((data.status.remaining_time.seconds || 0) * 1000);
                }
                if (typeof data.status.remaining_time.hours === 'number' && typeof data.status.remaining_time.minutes === 'number') {
                    const totalHours = data.status.remaining_time.hours; const minutes = data.status.remaining_time.minutes; const days = Math.floor(totalHours / 24); const remainingHoursInDay = totalHours % 24; let parts = []; if (days > 0) parts.push(`${days} hari`); if (remainingHoursInDay > 0) parts.push(`${remainingHoursInDay} jam`); if (minutes > 0) parts.push(`${minutes} menit`); formattedRemainingTime = parts.length > 0 ? parts.join(' ') : (remainingMillisecondsFromAPI > 0 ? "Kurang dari semenit" : "N/A");
                }
            }
            ...
            // PESAN PERINGATAN KEMBALI KE VERSI SEDERHANA (TANPA DEADLINE FOOTER)
        ...
        await handleServerProblem(phone, serverConfig, "InvalidResponse", { responseData: JSON.stringify(data), note: `Status API Bioauth tidak dikenal dari helper: "${apiStatusResult}"` });
...
async function startBioauthChecker() {
    if (isBioauthChecking) { await logMessage('Pengecekan Bioauth sudah berjalan. Melewati siklus ini.', 'BIOAUTH'); return; }
...
client.on('ready', async () => {
    ...
    // Mulai interval untuk memperbarui data epoch
    setInterval(updateGlobalEpochData, 1 * 60 * 1000); // Update data epoch setiap 1 menit
    await logMessage('Pengambilan data epoch global dijadwalkan setiap 1 menit.', 'SYSTEM');
...
async function main() {
    ...
    // Panggil sekali di awal untuk data awal
    await updateGlobalEpochData();
